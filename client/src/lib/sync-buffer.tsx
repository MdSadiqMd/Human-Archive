...
async function replayOp(op: BufferOp): Promise<void> {
	...
	switch (op.type) {
		case "submit":
			await api.queue.submit(op.assignmentId, op.data);
			break;
		case "skip":
			await api.queue.skip(op.assignmentId);
			break;
		case "update":
			await api.admin.updateAnnotation(op.annotationId, op.data);
			break;
		case "approve":
			await api.admin.approveAnnotation(op.annotationId, op.review_notes);
			break;
		case "reject":
			await api.admin.rejectAnnotation(op.annotationId, op.review_notes);
			break;
	}
}

export function SyncBufferProvider({
	children,
}: {
	...
	const add = useCallback((op: BufferOp) => {
		setEntries((prev) => {
			...
			const next = [
				...prev,
				{ id: crypto.randomUUID(), op, timestamp: Date.now() },
			];
			save(next);
			dispatchChanged();
			return next;
