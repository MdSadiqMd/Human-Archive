...
export function AnnotationCanvas({
	...
	const getHandleAtPoint = useCallback(
		(imgPoint: Point, box: BoundingBox): ResizeHandle | null => {
			const img = imageRef.current;
			if (!img) return null;
			...
			for (const h of handles) {
				if (
					Math.abs(imgPoint.x - h.x) < handleSizeNorm &&
					Math.abs(imgPoint.y - h.y) < handleSizeNorm
				) {
					return h.handle;
				}
			}
	...
	const getRotateHandleAtPoint = useCallback(
		(clientX: number, clientY: number, box: BoundingBox): boolean => {
			const canvas = canvasRef.current;
			const img = imageRef.current;
			...
			const cx =
				(box.x + box.width / 2) * imgW * transform.zoom + transform.offsetX;
	...
	const pointInBox = useCallback(
		(imgPoint: Point, box: BoundingBox): boolean => {
			if (!box.rotation) {
				return (
					imgPoint.x >= box.x &&
		...
		for (const box of boxes) {
			...
			if (isSelected && !readOnly) {
				...
				ctx.arc(
					rhX,
					rhY,
					ROTATION_HANDLE_RADIUS / transform.zoom,
					0,
					Math.PI * 2,
				);
				ctx.fillStyle = "#ffffff";
			...
			if (selectedId) {
				...
				if (selectedBox) {
					if (getRotateHandleAtPoint(e.clientX, e.clientY, selectedBox)) {
						...
						setRotateStartAngle(
							Math.atan2(-(imgPoint.y - centerY), imgPoint.x - centerX),
						);
						return;
					}
		...
		(e: React.MouseEvent<HTMLCanvasElement>) => {
			...
			if (mode === "resizing" && dragStart && originalBox && activeHandle) {
				...
				if (activeHandle.includes("w")) {
					x = Math.min(
						originalBox.x + originalBox.width - MIN_BOX_SIZE,
						originalBox.x + dx,
					);
				...
				if (activeHandle.includes("n")) {
					y = Math.min(
						originalBox.y + originalBox.height - MIN_BOX_SIZE,
						originalBox.y + dy,
					);
			...
			if (mode === "rotating" && dragStart && originalBox) {
				...
				if (rotateStartAngle !== null) {
					...
					onBoxesChange(
						boxes.map((b) => (b.id === originalBox.id ? newBox : b)),
					);
				}
				return;
			}
	...
	useLayoutEffect(() => {
		...
		const handler = (e: WheelEvent) => {
			...
			setTransform((t) => {
				...
				return {
					zoom: nz,
					offsetX: mouseX - wx * nz,
					offsetY: mouseY - wy * nz,
				};
			});
		};
	...
	return (
		<div
			ref={containerRef}
			className="w-full h-full relative bg-black/40 rounded-lg overflow-hidden"
		>
			<canvas
				ref={canvasRef}
