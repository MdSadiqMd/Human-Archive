...
export function SyncStatusBadge() {
	...
	return (
		<div
			className={`flex items-center gap-1.5 px-2 py-1 rounded-md text-xs ${cfg.color} ${cfg.bg}`}
		>
			<span
				className={`w-1.5 h-1.5 rounded-full ${dotColor} ${status === "syncing" ? "animate-pulse" : ""}`}
			/>
			<span>{cfg.label}</span>
			{pending > 0 && <span className="font-mono">({pending})</span>}
			{nearFull && (
				<span className="font-semibold text-orange-400">· nearly full</span>
			)}
		</div>
	);
}
