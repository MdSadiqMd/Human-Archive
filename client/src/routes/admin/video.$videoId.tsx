...
import { api, frameImageUrl, type Video, type User, type PaginatedFrames, type Assignment } from '#/lib/api'
...
export const Route = createFileRoute('/admin/video/$videoId')({ component: VideoDetailPage })

const LABELS = ['all', 'no_hands', 'easy', 'occluded', 'low_lighting', 'dexterous_pose'] as const
...
function VideoDetailPage() {
  ...
  const getFrameStatus = (frameId: string): 'pending' | 'completed' | 'skipped' | null => {
  ...
  async function handleAssign() {
    ...
    setSuccess('')
    try {
      const res = await api.admin.assignFrames(Array.from(selected), assigneeId)
  ...
  if (!video) return <div className="text-center py-12 text-muted-foreground">Loading…</div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
            <button onClick={() => navigate({ to: '/admin/videos' })} className="hover:text-foreground">Videos</button>
            <span>/</span>
            <span className="text-foreground font-mono text-xs">{video.stem}</span>
          </div>
          <h1 className="text-2xl font-semibold text-foreground">{video.stem}</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <div className="border border-border rounded-lg p-3">
          ...
          <div className="text-lg font-semibold mt-0.5">{Math.floor(video.duration_s / 60)}m {Math.floor(video.duration_s % 60)}s</div>
        </div>
        <div className="border border-border rounded-lg p-3">
        ...
        <div className="border border-border rounded-lg p-3">
          ...
          <div className="text-lg font-semibold mt-0.5">{video.total_frames}</div>
        </div>
        <div className="border border-border rounded-lg p-3">
          <div className="text-xs text-muted-foreground">Sampled</div>
          <div className="text-lg font-semibold mt-0.5">{video.sampled_frames}</div>
        </div>
        <div className="border border-border rounded-lg p-3">
          <div className="text-xs text-muted-foreground">Status</div>
          <div className="text-lg font-semibold mt-0.5 capitalize">{video.status}</div>
        </div>
      </div>
      ...
      {/* Label filter */}
      <Tabs value={activeLabel} onValueChange={v => { setActiveLabel(v as Label); setSelected(new Set()) }}>
        <TabsList>
          {LABELS.map(label => (
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          >
            {frames && selected.size === frames.frames.length && frames.frames.length > 0 ? 'Deselect all' : 'Select all'}
          </button>
          {frames && (
            ...
            return (
              <div
                ...
                className={`relative border rounded-lg overflow-hidden cursor-pointer transition-colors ${
                  selected.has(frame.id) ? 'border-primary ring-2 ring-primary/30' : 'border-border hover:border-muted-foreground/30'
                }`}
              ...
              >
                <div className="aspect-[4/3] bg-muted relative group">
                  ...
                  {status && (
                    <div className={`absolute top-1 left-1 px-1.5 py-0.5 rounded text-[9px] font-medium ${
                      ...
                      status === 'skipped' ? 'bg-yellow-500/90 text-black' :
                ...
                <div className="p-1.5 space-y-0.5">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline" className={`text-[10px] px-1 py-0 ${labelColors[frame.label] || ''}`}>
                      {frame.label.replace(/_/g, ' ')}
                    </Badge>
                  </div>
                  {frame.num_hands > 0 && (
                    <div className="text-[10px] text-muted-foreground">
                      {frame.num_hands} hand{frame.num_hands !== 1 ? 's' : ''} · {frame.hand_evidence.toFixed(2)}
                    </div>
                  )}
                </div>
              </div>
        ...
        {frames && frames.frames.length === 0 && (
          <p className="text-center py-12 text-muted-foreground">No frames found for this filter.</p>
        )}
      </div>
      ...
      {frames && frames.total_pages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <Button
            ...
            onClick={async () => {
              const f = await api.admin.listFrames(videoId, { label: activeLabel, page: frames.page - 1, per_page: 200 })
              setFrames(f)
              setSelected(new Set())
            }}
          >
