...
function VideosPage() {
  ...
  async function handleIngest() {
    ...
    try {
      ...
      setIngestMsg(`Ingested ${res.videos} video(s) with ${res.frames} frame(s)`)
      await fetchVideos()
    } catch (err: any) {
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center text-muted-foreground py-12">
                  No videos ingested yet. Run the pipeline then ingest the output directory.
                </TableCell>
              </TableRow>
            ) : (
              videos.map((video) => (
                <TableRow key={video.id}>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {Object.entries(video.label_counts || {}).map(([label, count]) => (
                        <Badge key={label} variant="outline" className={`text-[11px] ${labelColors[label] || ''}`}>
                          {label}: {count}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <TableCell className="text-right">
                    <Button asChild variant="outline" size="sm">
                      <Link to="/admin/video/$videoId" params={{ videoId: video.id }}>
                        View
                      </Link>
                    </Button>
                  </TableCell>
