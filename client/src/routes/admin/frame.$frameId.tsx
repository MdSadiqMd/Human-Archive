...
import { api, frameImageUrl, type Frame, type AnnotationWithMeta, type Video, type BoundingBox } from '#/lib/api'
...
export const Route = createFileRoute('/admin/frame/$frameId')({ component: FrameDetailPage })
...
function FrameDetailPage() {
  ...
  const loadAnnotation = useCallback(async (f: Frame) => {
    try {
      ...
      setAnnotation(a)
      if (a) {
        const hasCorrection = a.corrected_bounding_boxes && a.corrected_bounding_boxes.length > 0
        setEditedBoxes(hasCorrection ? a.corrected_bounding_boxes : a.bounding_boxes)
  ...
  function effectiveBoxes(a: AnnotationWithMeta) {
    return a.corrected_bounding_boxes && a.corrected_bounding_boxes.length > 0
      ? a.corrected_bounding_boxes : a.bounding_boxes
  }

  function effectiveNoHands(a: AnnotationWithMeta) {
    return a.corrected_bounding_boxes && a.corrected_bounding_boxes.length > 0
      ? a.corrected_no_hands : a.no_hands
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isNetworkError(err)) {
        bufferAdd({ type: 'update', annotationId: annotation.id, data: { no_hands: editedNoHands, bounding_boxes: editedBoxes, notes: editedNotes, review_notes: reviewNotes } })
        setReviewMode(false)
        setReviewNotes('')
      } else {
  ...
  async function handleApprove() {
    ...
    try {
      ...
      if (isNetworkError(err)) {
        bufferAdd({ type: 'approve', annotationId: annotation.id, review_notes: reviewNotes })
        setReviewNotes('')
      } else {
  ...
  async function handleReject() {
    ...
    try {
      ...
      if (isNetworkError(err)) {
        bufferAdd({ type: 'reject', annotationId: annotation.id, review_notes: reviewNotes })
        setReviewNotes('')
      } else {
  ...
  }, [reviewMode, handleUndoFrame, handleRedoFrame, handleSendToBackFrame, handleBringToFrontFrame, handleDeleteSelectedFrame])

  if (error) {
    return (
      <div className="p-6">
        <div className="text-destructive mb-4">{error}</div>
        <Button variant="outline" onClick={() => navigate({ to: '/admin/reviews' })}>
          Back to Reviews
        </Button>
      </div>
    )
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
            ...
            <span>/</span>
            <button
              onClick={() => navigate({ to: '/admin/video/$videoId', params: { videoId: video.id } })}
        ...
        <div className="flex items-center gap-2">
          <SyncStatusBadge />
          {annotation && (
            <Badge variant="outline" className={statusColors[annotation.review_status] || 'bg-yellow-500/15 text-yellow-400'}>
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          <div className="h-[500px] border border-border rounded-lg overflow-hidden">
            <AnnotationCanvas
              ...
              boxes={reviewMode ? editedBoxes : (annotation ? effectiveBoxes(annotation) : [])}
          ...
          {reviewMode && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>·</span>
              <span>Press <kbd className="px-1 py-0.5 bg-muted rounded text-[10px]">1</kbd> L / <kbd className="px-1 py-0.5 bg-muted rounded text-[10px]">2</kbd> R for selected box</span>
            </div>
          )}
        </div>
                ...
                <div className="space-y-2 text-sm">
                      ...
                      {effectiveBoxes(annotation).map((box, i) => (
                        <div key={box.id} className="flex items-center gap-2 text-xs">
                          ...
                          <span className="text-muted-foreground">
                            Box {i + 1}: ({(box.x * 100).toFixed(0)}%, {(box.y * 100).toFixed(0)}%)
                            {(box.width * 100).toFixed(0)}×{(box.height * 100).toFixed(0)}%
                          </span>
                        </div>
                      ))}
                      {annotation.review_status === 'corrected' && annotation.corrected_bounding_boxes?.length > 0 && (
                        <p className="text-[10px] text-blue-400 mt-1">Showing corrected annotation</p>
                      )}
                    </div>
                  )}
                  <div className="flex justify-between pt-2 border-t border-border">
                    <span className="text-muted-foreground">Annotator</span>
                    <span className="text-xs">{annotation.annotator_email}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {annotation.notes && (
                    <div className="pt-2 border-t border-border">
                      <span className="text-muted-foreground block mb-1">Annotator notes</span>
                      <p className="text-xs bg-muted/50 rounded p-2">{annotation.notes}</p>
                    </div>
                  )}
                  {annotation.review_status && annotation.review_status !== 'pending' && (
                    <div className="pt-2 border-t border-border space-y-1">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Review status</span>
                        <Badge variant="outline" className={`text-[10px] ${statusColors[annotation.review_status] || ''}`}>
                      ...
                      {annotation.reviewed_by_email && (
                        <div className="flex justify-between">
                          ...
                          <span className="text-xs">{annotation.reviewed_by_email}</span>
                        </div>
                      )}
                      {annotation.review_notes && (
                        <div>
                          <span className="text-muted-foreground text-[10px] block mb-0.5">Review notes</span>
                          <p className="text-xs bg-muted/50 rounded p-2">{annotation.review_notes}</p>
                        </div>
                      )}
                    </div>
                  )}
