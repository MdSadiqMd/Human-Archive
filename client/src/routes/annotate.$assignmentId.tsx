...
export const Route = createFileRoute("/annotate/$assignmentId")({
...
function AnnotatePage() {
		...
		try {
			...
			if (idx >= 0) {
				...
				navigate({
					...
					params: { assignmentId: items[0].assignment_id },
					replace: true,
				});
			}
		} catch {
	...
	function pushHistory(newBoxes: BoundingBox[], newSelectedId: string | null) {
		...
		historyRef.current = [
			...
			{ boxes: newBoxes, selectedId: newSelectedId },
		];
		...
		try {
			...
			if (isNetworkError(err)) {
				bufferAdd({
					...
					data: { no_hands: noHands, bounding_boxes: boxes, notes },
				});
				setSubmitting(false);
				await advance();
			} else {
	...
	async function advance() {
		...
		if (next.length > 0) {
			...
			navigate({
				to: "/annotate/$assignmentId",
				params: { assignmentId: nextItem.assignment_id },
				replace: true,
			});
		} else {
			setItem(null);
				...
				<div className="max-w-7xl mx-auto px-4 h-12 flex items-center justify-between">
					...
					<div className="flex items-center gap-4">
						<div className="flex items-center gap-2 text-xs text-muted-foreground">
							<span>
								Frame {currentIdx + 1} of {queue.length + currentIdx + 1}
							</span>
							<span>·</span>
							<span>
								{progress?.completed ?? 0} / {progress?.total ?? 0} ({pct}%)
							</span>
						</div>
			...
			<main className="flex-1 flex">
				<div className="flex-1 p-4">
					...
					<p className="text-xs text-muted-foreground mt-2 text-center">
						{item.video_stem} · Frame #{item.frame_index} · t=
						{item.timestamp_s.toFixed(2)}s
					</p>
				</div>
